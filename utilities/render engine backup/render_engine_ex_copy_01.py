...
class RenderEngineEx():
    ...
    def get_perspective_offset(self, surface_pos, focus_point, layer_index, stack_object_height, total_layers=None):
        ...
        # Cap distance to avoid extreme distortion
        screen_w, screen_h = self.screen_size
        max_distance = max(screen_w, screen_h)
        distance = min(direction.length(), max_distance)

        # Normalize by axis (prevents vertical bias)
        # Do NOT normalize the vector first—scale components by their axis.
        sx = direction.x / (screen_w / 2)
        sy = direction.y / (screen_h / 2)

        # Optional: re-normalize to keep unit-ish length in normalized space
        d = pygame.Vector2(sx, sy)
        if d.length() != 0:
            d = d.normalize()

        # Scale per layer
        radial_mag = self.perspective_strength * (stack_object_height / max(total_layers, 1))
        offset_x = d.x * radial_mag * layer_index * (distance / (screen_w / 2))
        offset_y = d.y * radial_mag * layer_index * (distance / (screen_h / 2))
    ...
    def slice_and_stack(self, x, y, sprite_sheet, local_rotation_angle = 0, height=1.0, overdraw_px=0, debug = False, debug_hitbox = None, fixed_canvas : pygame.Surface | None = None, rotating = 0):
        
        slices = self.get_slices_cached(sprite_sheet, pad=0)
        ...
        for i, layer in enumerate(slices):
            ...
            if rotating:
                ...
                layer = pygame.transform.rotate(layer, self.camera_angle)
            else: 
                continue 
            '''
            ...
            cx = (canvas.get_width()  - layer.get_width())  // 2
            cy = (canvas.get_height() - layer.get_height()) // 2
            canvas.blit(layer, (cx, cy))
            ...
            sp = self.world_to_screen(perspective_offset_position)
